...
const getFontFamily = ({fontWeight, isFontTypeEnglish = false} : {fontWeight: FontWeight; isFontTypeEnglish: boolean;}) => 
  isFontTypeEnglish ? "TitanOne" : `SUIT-${fontWeight.slice(0, 1).toUpperCase().concat(fontWeight.slice(1))}`;


const getLineHeight = (fontSize: number, lineHeightRatio = 1.5) => fontSize * lineHeightRatio;
